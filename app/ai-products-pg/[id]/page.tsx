...
import { ArrowLeft, Brain, DollarSign, ShoppingCart, ExternalLink, CheckCircle, AlertCircle, Star, TrendingUp, Users, Package, Loader2 } from 'lucide-react'
...
export default function AIProductDetailPG() {
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        setLoading(true)
        const response = await fetch(`/api/ai-products-pg/${productId}`)
        if (!response.ok) {
              ...
              <div className="space-y-4">
                ...
                <div className="bg-purple-50 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-purple-600">Best Price</span>
                    <span className="text-2xl font-bold text-purple-600">{formatPrice(product.min_price)}</span>
                  </div>
